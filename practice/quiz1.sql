...
SELECT * FROM book WHERE book_title LIKE '%자바%' ORDER BY book_title DESC;
...
SELECT cart.buyer, cart.buy_price, member.member_name
FROM cart, member WHERE cart.buyer = member.member_id;
# 문제) cart테이블에서 회원이 구매한 도서의 할인되기 전의 가격과 할인율 가져오기
SELECT b.book_title, b.book_price, b.book_discount_rate, c.buy_price
FROM cart c, book b WHERE c.book_no = b.book_no;
# 문제) cart테이블에서 회원이 구매한 도서의 할인되기 전의 가격이 2만원 이상인 도서정보와 할인율 가져오기
SELECT b.book_title, b.book_price, b.book_discount_rate FROM cart c, book b
